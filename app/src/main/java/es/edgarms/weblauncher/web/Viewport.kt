...
object Viewport {
     ...
     * when the viewport changes, but it will not pull a page back from a scale
     * the reader chose by pinching; pinning both ends forces the new layout zoom,
     * and relaxing them hands the pinch back at the scale just asserted.
     *
     * The pending relax is cancelled first, and that is not tidiness: each
     * injection closes over ITS width and scale, so a timer from the previous one
     * firing after this one writes the old viewport back. Two taps on the zoom
     * inside 50 ms are enough.
     */
    fun script(base: Int, desktop: Boolean, zoom: Int): String {
        ...
        return """
            (function () {
              ...
              window.__wlFree = setTimeout(function () {
